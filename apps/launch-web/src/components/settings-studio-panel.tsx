  ...
  return (
    <div className={`neb-settings-row${faint ? " faint" : ""}`}>
      ...
      {action && actionLabel
        ...
        : (
          <span aria-hidden="true" className="neb-settings-action placeholder">
            —
          </span>
        )}
    </div>
      ...
      <div className="neb-settings-groups">
        ...
        <SettingsGroup label="Access">
          ...
          <SettingsRow
            ...
            value={shortcutPreferences?.shortcutsEnabled === false
              ? "off"
              : "on"}
          />
        </SettingsGroup>
        ...
        <button
          className="neb-settings-signout"
          onClick={onSignOut}
          type="button"
        >
          Sign out
...
function ModelKeysSettings({
  ...
  const connected = (byok?.providers ?? []).filter((provider) =>
    provider.configured
  );
  const available = (byok?.providers ?? []).filter((provider) =>
    !provider.configured
  );
  ...
  return (
    <SettingsBack onBack={onBack} title="Model keys">
      <p className="neb-settings-subtitle">
        Your keys power <code>galactic.ai()</code>{" "}
        directly. Galactic stores them encrypted, never resells inference, and
        never bills you for it.
      </p>
      <SettingsGroup label="Connected">
        ...
        {byok && connected.length === 0
          ? (
            <div className="neb-settings-empty-row">
              No model keys connected
            </div>
          )
          : null}
      </SettingsGroup>
      <SettingsGroup label="Available">
        {available.map((provider) => (
          <div className="neb-settings-provider-row" key={provider.id}>
            <span>
              ...
              <small>
                {provider.description ?? "Connect this model provider"}
              </small>
            </span>
            <button
              className="neb-settings-action"
        ...
        {!byok
          ? <div className="neb-settings-empty-row">Loading providers…</div>
          : null}
      </SettingsGroup>
    </SettingsBack>
  );
    ...
    >
      ...
      >
        ...
        <ByokCredentialForm
          ...
          requiredOperations={provider.validation?.operations ?? ["generate"]}
        />
      </div>
    </div>,
    document.body,
...
function GalacticKeysSettings({
  ...
  const revokeIdle = async () => {
    ...
    try {
      ...
      onToast(
        `Revoked ${idle.length} idle token${idle.length === 1 ? "" : "s"}`,
      );
    } catch (error) {
      setError(error instanceof Error ? error.message : String(error));
    }
        ...
        ? (
          <div className="neb-settings-new-token">
            ...
            <div className="neb-settings-new-token-actions">
              ...
              <button onClick={() => setPlaintext("")} type="button">
                Dismiss
              </button>
            </div>
          </div>
  ...
  return (
    <SettingsBack onBack={onBack} title="Keyboard shortcuts">
      ...
      <div className={`neb-settings-shortcut-map${enabled ? "" : " disabled"}`}>
        ...
        <SettingsGroup label="Jump to an agent">
          {rows(agentShortcuts)}
        </SettingsGroup>
      ...
      <p className="neb-settings-footnote">
        Nothing fires while you type, compose with an IME, use a dialog, or work
        inside an embedded Interface.
      </p>
    </SettingsBack>
  );
}
...
export function SettingsStudioPanel({
  ...
  return (
    <section
      className="neb-inline-panel neb-settings-panel"
      aria-label="Settings"
    >
      {error
        ? <p className="neb-settings-error" role="alert">{error}</p>
        : null}
      {page === "root"
        ? (
          <SettingsRoot
