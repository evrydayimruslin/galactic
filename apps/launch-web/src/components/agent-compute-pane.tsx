...
function humanizeTool(tool: string): string {
  return tool.replaceAll(/[._-]+/gu, ' ').replaceAll(/\b\w/gu, (value) => value.toUpperCase());
}
...
export function AgentComputePane({
  ...
  const [featureState, setFeatureState] = useState<ComputeFeatureState>('loading');
  const [snapshot, setSnapshot] = useState<LaunchComputeSettingsResponse | null>(null);
    ...
    void (async () => {
      try {
        ...
        if (!isValidSettingsResponse(response)) {
          throw new Error('Compute settings returned an invalid authority view.');
        }
        if (!mounted) return;
        ...
        try {
          const runResponse = await launchApi.agentComputeRuns(locator, { limit: 50 });
          if (mounted) {
            setRuns(runResponse.runs);
            setNextRunCursor(runResponse.next_cursor ?? null);
          }
        } catch (reason) {
          if (mounted) setError(`Run history is unavailable: ${errorMessage(reason)}`);
        }
      } catch (reason) {
        if (!mounted) return;
        setFeatureState(isComputeEndpointUnavailable(reason) ? 'unavailable' : 'error');
        if (!isComputeEndpointUnavailable(reason)) setError(errorMessage(reason));
      }
    })();
    return () => {
      mounted = false;
  ...
  const bindingSummary = (name: string): LaunchComputeSecretBindingSummary | undefined =>
    snapshot?.settings.secretBindings.find((binding) => binding.name === name);

  const toggleSecret = (name: string) => {
    if (!draft) return;
    const selected = draft.secretBindings.some((binding) => binding.name === name);
    updateDraft({
      ...draft,
      secretBindings: selected
        ...
        : [...draft.secretBindings, {
          name,
          delivery: bindingSummary(name)?.delivery ?? defaultSecretDelivery(name),
        }],
    });
  };
  ...
  const updateLimit = (
    key: 'maxTimeoutMs' | 'maxConcurrency' | 'maxArtifactBytes' | 'maxArtifacts',
  ) =>
  ...
  const removeAuthorityRule = (index: number) => {
    ...
    updateDraft({
      ...draft,
      authorityRules: draft.authorityRules.filter((_, candidateIndex) => candidateIndex !== index),
    });
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!isValidSettingsResponse(response)) {
        throw new Error('Compute settings update returned an invalid authority view.');
      }
      setSnapshot(response);
      setDraft(computeSettingsDraft(response.settings));
      setConfirmed(false);
  ...
  const cancel = async (run: LaunchComputeRunSummary) => {
    ...
    if (!window.confirm(`Cancel Compute run ${run.runId}? The disposable body will be stopped.`)) {
      return;
    }
    ...
    try {
      ...
      setRuns((current) =>
        current.map((candidate) => candidate.runId === run.runId ? next : candidate)
      );
    } catch (reason) {
      setError(errorMessage(reason));
    } finally {
  ...
  if (featureState === 'loading') {
    return (
      <section className='neb-modal-pane active'>
        ...
        <p className='neb-ov-note'>Loading the owner-authorized Compute boundary…</p>
      </section>
    );
  }
    ...
    return (
      <section className='neb-modal-pane active'>
        ...
        <div className='neb-compute-gate' role='status'>
          ...
          <p>
            The control-plane endpoint is absent, so no Compute controls or cancellation actions are
            enabled.
          </p>
        </div>
      </section>
    ...
    return (
      <section className='neb-modal-pane active'>
        ...
        <p className='neb-error-note' role='alert'>
          Compute is fail-closed: {error || 'its authority view could not be loaded.'}
        </p>
      </section>
    );
  }
  ...
  return (
    <section className='neb-modal-pane active'>
      ...
      <p className='neb-ov-note top-note'>
        Disposable Linux bodies for this Agent. These owner settings can only narrow the currently
        live release; the control plane enforces the final boundary on every run.
      </p>
      {error ? <p className='neb-error-note' role='alert'>{error}</p> : null}
      {saved ? <p className='neb-release-success' role='status'>{saved}</p> : null}
      ...
      <section className='neb-ov-section'>
        ...
        {!ceiling.enabled
          ? (
            <div className='neb-compute-gate'>
              <strong>The live release does not declare galactic.compute().</strong>
              <p>
                Add <code>compute:exec</code> and a <code>{COMPUTE_PROFILE}</code>{' '}
                manifest ceiling in a reviewed release before it can be enabled here.
              </p>
            </div>
          )
          : null}
        <label className='neb-compute-check primary'>
          <input
            ...
            onChange={(event) => updateDraft({ ...draft, enabled: event.currentTarget.checked })}
            type='checkbox'
          />
          <span>
            <strong>Enable Compute</strong>
            <small>Allow this Agent's approved functions to start one-shot bodies.</small>
          </span>
        </label>
      ...
      <section className='neb-ov-section'>
        ...
        <p className='neb-ov-note'>
          These labels disclose the environment capabilities this release expects for dependency
          resolution and audit. They neither install packages nor enforce which binaries a Linux
          body may execute.
        </p>
        ...
        {ceiling.tools.length === 0
          ? <p className='neb-ov-note'>The live release declares no Compute dependencies.</p>
          : null}
      </section>
      ...
      <section className='neb-ov-section'>
        ...
        <p className='neb-ov-note'>
          Map only secret names declared by this release. Values are managed write-only in Variables
          and are never returned here.
        </p>
        {declaredSecrets.map((name) => {
          const binding = draft.secretBindings.find((candidate) => candidate.name === name);
              ...
              <label className='neb-compute-check'>
                ...
                <span>
                  ...
                  <small className={summary?.configured ? 'configured' : ''}>
                    {summary?.configured ? 'value configured' : 'value not configured'}
                  </small>
                </span>
              </label>
              {binding
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        ...
                        updateSecretDelivery(
                          name,
                          kind === 'file'
                            ? { kind: 'file', path: `/run/galactic/secrets/${name}` }
                      ...
                      onChange={(event) =>
                        updateSecretDelivery(
                          ...
                          binding.delivery.kind === 'env'
                            ? { kind: 'env', envName: event.currentTarget.value }
      ...
      <section className='neb-ov-section'>
        ...
        <p className='neb-ov-note'>
          Choose one exact Galactic function, or one exact Agent and function, for each calling
          function. No wildcard grant exists.
        </p>
            ...
            <div className='neb-compute-authority-rule' key={index}>
              ...
              <label>
                ...
                <select
                  ...
                  value={rule.action}
                >
                  <option value='platform.call'>Galactic platform function</option>
              ...
              {rule.action === 'agents.call'
                ? (
                  <label>
                    ...
                    <input
                      aria-label={`Authority rule ${index + 1} target Agent UUID`}
                      className='neb-edit-input'
                      disabled={!draft.enabled}
                      onChange={(event) =>
                        updateAuthorityRule(index, {
              ...
              <label>
                ...
                <input
                  ...
                  placeholder={rule.action === 'platform.call' ? 'gx.upload' : 'functionName'}
                  spellCheck={false}
      ...
      <section className='neb-compute-confirm' aria-label='Confirm Compute authority'>
        ...
        <label className='neb-compute-check'>
          ...
          <span>
            ...
            <small>
              Compute bodies may browse the internet, receive the mapped secrets, and exercise only
              the exact platform or Agent rules shown above.
            </small>
          </span>
        </label>
        <button
      ...
      <section className='neb-ov-section neb-compute-runs'>
        <div className='neb-compute-section-head'>
          ...
          <button
            className='neb-btn-sm'
            onClick={() => void loadRuns().catch((reason) => setError(errorMessage(reason)))}
            type='button'
          >
            Refresh
          </button>
        ...
        {runs.length === 0 ? <p className='neb-ov-note'>No Compute runs yet.</p> : null}
        {nextRunCursor
          ? (
            <button
              className='neb-btn-sm'
          ...
          : null}
        <p className='neb-ov-note'>
          Infrastructure failures and settlement reconciliation also appear in this Agent's Alerts.
        </p>
      </section>
    </section>
  );
...
function ComputeRunRow({
  ...
  return (
    <article className={`neb-compute-run status-${run.status}`}>
      <div className='neb-compute-run-head'>
        <div>
          <strong>{run.functionName}</strong>
          <span>{run.agentName} · {run.runId}</span>
        </div>
        <span className='neb-compute-run-status'>{run.status.replaceAll('_', ' ')}</span>
      </div>
      <div className='neb-compute-run-grid'>
        ...
        <span>
          <small>Actual</small>
          {run.usage.actual === null ? 'pending' : `${run.usage.actual} ${run.usage.unit}`}
        </span>
        <span>
          <small>True-up</small>
          {run.usage.trueUp === null
            ? 'pending'
            : `${run.usage.trueUp >= 0 ? '+' : ''}${run.usage.trueUp} ${run.usage.unit}`}
        </span>
        <span>
      ...
      <div className='neb-compute-run-links'>
        {run.receiptId
          ? receiptUrl
            ? <a href={receiptUrl} rel='noreferrer' target='_blank'>Receipt {run.receiptId}</a>
          ...
          return url
            ? (
              <a href={url} key={artifact.id} rel='noreferrer' target='_blank'>
                {artifact.name} · {formatBytes(artifact.sizeBytes)} · available until{' '}
