...
|----|-------|-----------|------|--------|
| WO-F1 | Anonymous claimable handoff + pairing page + hero | — | 2 PRs | **PR A built** (API + migration: provisional owners, anonymous mint, stages-only pairing read, atomic claim RPC, reaper). Divergences from the original order, adopted: (1) pairing lives in a dedicated `funnel_sessions` table keyed by pairing code — not columns on `builder_handoff_sessions` — so the code survives `resume` re-mints (the funnel row swaps its `handoff_session_id`); (2) the reaper runs opportunistically at mint (bounded batch of 25, failure-silent) instead of a cron — same semantics, zero new scheduled infrastructure, promotable to cron later. **PR B built** (web: `/b/:code` watch page polling the sanitized projection with a stages ledger, gone/stale states, and a claimed voice; homepage hero terminal block — command, copy button, price + no-account promise — above the untouched plan card; a remembered-build card adopts the visitor's own pairing onto the pre-auth roster via localStorage). Divergence: client-side funnel *event emission* is deferred — the session ledger's stage timestamps already carry the time-to-first-held-card KPI server-side, and the client event channel lands with WO-F3's checkout states where instrumentation earns its keep. |
| WO-F2 | `galacticconnection new` | WO-F1 | 1–2 PRs | **built** (one PR: pure-Node `new` + `resume` in the bin — plan + boundary question, anonymous mint, MCP client wiring reused from setup, brief with offer-to-launch Claude Code (never auto-run), watch loop mirroring ledger stages; `POST /funnel/pairings/:code/resume` API re-mints under the same pairing for the same provisional owner, refusing claimed/elapsed funnels, same per-IP ceiling. A funnel credential fills an empty bridge slot or replaces a prior funnel credential; a real account key is never clobbered — pinned by tests). |
| WO-F3 | Checkout at the card + claim consolidation | WO-F1 | 1–2 PRs | **built** (one PR): `POST /funnel/pairings/:code/checkout` mints a **customerless** subscription checkout through the existing hardened attempt ledger (Link captures + OTP-verifies the email; funnel metadata on the session; subscription-level `user_id` metadata deliberately omitted so subscription events resolve via the customer mapping). Webhook completion (`maybeCompleteFunnelCheckout`): **fresh email → the provisional row is promoted in place** (email + member + stripe_customer_id; user id stable = ordering-proof) and the build claims itself; **existing email → the account gains the customer mapping only** (never overwriting a different live Stripe identity) and the agent waits for that account's own sign-in claim (decision 9; Link's OTP counts as payment-identity verification, not agent-attach verification). Card actions wired: signed-out Approve → checkout ("Approve and let it finish — $20/month"); signed-in → claim-into-fleet; Deny/Edit stay free behind sign-in. Divergences: checkout mints at the card click rather than idle-at-exact-tested (identical UX, no abandoned Stripe sessions); the client funnel-event channel is folded into the checkout attempt ledger + session stages rather than a separate events table. |
| WO-F4 | Device authorization grant → `gx_` keys | — | 1 PR | **built** (one PR): `device_authorizations` migration (single-use XXXX-XXXX user codes, hashed device codes, ten-minute CHECK-pinned window); three routes (anonymous mint, anonymous poll with exactly-once consume-before-mint, account-session approve); `/device` page (?code= prefills, approval is ALWAYS an explicit click in a session — never from a URL alone; the key never appears in the browser); `galacticconnection login` polls and lands the standard-scope gx_ key into `~/.galactic/config.json`, clearing any funnel credential and rewiring MCP clients. Key issuance rides the existing `createToken` — which requires an active membership, making device login precisely the post-payment CLI upgrade the journey prescribes (non-members get a clear 403). |
| WO-F5 | Policy lane: `gx.policy`, starter policy, trial execution context | WO-F1 (envelope re-parenting) | 2–3 PRs | **PR A built** (`gx.policy` registry capability — read / attach_template / propose, never approve; `agent_policy_drafts` migration; skills-doc section). Implementation truths adopted: the held card runs on the **overlay** — attach_template sets `ask` via the gate's own `setFunctionPolicy` (CAS + `set_by` attribution) for every declared function in the guarded groups (**spend, external_side_effect**), classified server-side by `classifyFunctionConsequence` from declarations, so group scoping trusts the manifest rather than agent-supplied lists; immutable policy-set versions still mint only at owner readback (drafts are the parking lane). attach happens post-upload (declarations must exist); the starter "template" needs no compiled artifact in v1 — the overlay IS the enforcement, so the pre-compiled-artifact plan collapses into something simpler and stronger. PR B built: "Run it once" composes P3 primitives and nothing else — a job created held-for-approval (consumer-invisible; approval resumes through the existing exactly-once filter) plus a real approval envelope filed against the starter policy's ask row. **Real hold, deferred execution**: nothing runs pre-claim; execution reality begins after claim + membership + the untouched deploy boundary, at resume. Envelope TTL is already 7 days (APPROVAL_TTL_MS) — the return-window alignment cost zero changes. Pairing read enriches with the held card + trial-run count (ceiling 3/Agent); the pairing page renders the card (Approve routes to claim; Deny/Edit stay free, sign-in required) and the Run-it-once button post-upload. |
