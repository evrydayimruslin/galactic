...
## Historical capability changes (C1–C5)

- **C1 — Reintroduce BYOK + credit-denominated inference.** Both already run
...
- **C2 — Remove Widgets from launch scope entirely.** Facade endpoints,
...
- **C3 — Fold Skills into Functions.** First-class monetized skills (manifest
...
- **C4 — Rename Tools → Agents.** Presentation-layer rename only. The DB
  entity remains `apps` (per the handoff rule "Tools = apps"); MCP protocol
  vocabulary (`tools/list`, `tools/call`) is untouched.
- **C5 — Per-function cross-agent permissions.** Deploy/publish reads a
...
3. **C5 composition: the per-Agent grant is authoritative.** A user-granted
...
4. **Removal depth for C2/C3: delete from launch scope, keep the basin
...
- **Phase 0 (this change):** decision records; scope-doc rewrite; launch web
  ...
  cookie + silent client refresh) so write-heavy C1/C5 surfaces don't die at
  the ~1h JWT cliff; desktop decoupling of shared auth infra (embed-bridge
  audience now required; tauri CORS origins removed).
- **Phase 1:** C3 (skills → functions convention) + C2 (widget removal),
  together. Hard ordering constraint: design the C5 manifest
...
- **Phase 2:** C1 — BYOK settings surface over existing `/api/user/byok`,
...
- **Phase 3:** C4 — Tools → Agents rename (`/tools/:slug` → `/agents/:slug`
...
- **Phase 4:** C5 — manifest declaration + grant table/inbox/UI +
...
- **Phase 4.5:** C5 reactive layer — cross-Agent pub/sub event bus. An Agent
...
- "Monthly spend caps per function" do NOT exist today:
  ...
  own tests. C5 builds real credit-denominated caps; nothing should be wired
...
- Production `ul.codemode` executes library-app functions in-process with no
  ...
  (`api/runtime/dynamic-executor.ts`, `platform-mcp.ts` codemode path). C5
...
- The connected-agent permission layer (migration `20260606150000`,
  ...
  is live end-to-end and becomes the "caller-permissions" base for C5.
- BYOK silently activates for users who configured it pre-launch:
  ...
  C1 must surface billing mode explicitly.
