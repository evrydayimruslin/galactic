...
| WO-6 | Concept graph v1 — brackets, glossary, `about()`/`suggest()` | WO-5 (store + binding patterns) | 3 PRs | **PR A + B + C landed** — full data plane, agent plane (binding/template v27/docs), and the Studio glossary (Knowledge pane second act: list, concept pages with layer-labeled backlinks, owner descriptions). WO-6.1 landed: gx.concepts registry capability (demoted tier; builder_handoff may describe, agent-attributed) + dedicated sandbox wiring test. D1 tier-1 remains, with its one real design question now named: row identity for derived mentions at the DatabaseBinding chokepoint (inserts carry last_row_id; structured updates return change counts, not ids — decide surface_id semantics before hooking, or updates leave stale mentions). |
