...
const TOOLCHAIN_SCRIPT = [
  ...
  'const expectedGalacticCliVersion = load("node:fs").readFileSync("/opt/galactic/image-metadata/galactic-cli-version.txt", "utf8").trim();',
  "const observed = {",
  ...
  "};",
  'const expected = { python: "3.13.14", npm: "12.0.1", deno: "2.9.3", galactic_cli: expectedGalacticCliVersion, playwright: "1.62.0-alpha-2026-07-20", chromium: "152.0.7977.8" };',
...
const HTTPS_EGRESS_SCRIPT = [
  ...
  "probe_denied() {",
  '  target="$1"',
  '  transport_fallback="$2"',
  "  set +e",
  '  denied_code="$(curl --silent --location --output /dev/null --write-out \'%{http_code}\' --connect-timeout 3 --max-time 5 "$target")"',
  '  curl_status="$?"',
  "  set -e",
  ...
  "    return 0",
  "  fi",
  "  # Loopback/link-local may fail before ContainerProxy. Only fixed IP",
  "  # probes may accept the narrow curl transport-denial outcomes below.",
  '  test "$transport_fallback" = "literal" || exit 92',
  '  test "$denied_code" = "000" || exit 92',
  '  case "$curl_status" in',
  "    7|28|52|56) printf 'transport_exit_%s' \"$curl_status\" ;;",
  "    *) exit 92 ;;",
  "  esac",
  "}",
  'private_denial_mode="$(probe_denied http://127.0.0.1/ literal)"',
  'metadata_denial_mode="$(probe_denied http://169.254.169.254/latest/meta-data/ literal)"',
  'control_plane_denial_mode="$(probe_denied https://api.connectgalactic.com/health policy)"',
  'printf \'{"schema_version":1,"scenario":"https_egress_boundaries","verified":true,"public_https_ok":true,"private_denied":true,"private_denial_mode":"%s","metadata_denied":true,"metadata_denial_mode":"%s","control_plane_denied":true,"control_plane_denial_mode":"%s"}\\n\' "$private_denial_mode" "$metadata_denial_mode" "$control_plane_denial_mode"',
].join("\n");
