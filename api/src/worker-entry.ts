...
export { ConceptsBinding } from "./bindings/concepts-binding.ts";
  ...
  async fetch(
    ...
    try {
      ...
      for (const [key, value] of Object.entries(standardHeaders)) {
        ...
        if (
          key === "X-Frame-Options" && url.searchParams.get("embed") === "1"
        ) continue;
        response.headers.set(key, value);
      }
      if (!isHttpRoute) {
...
async function runMinuteJobs(): Promise<void> {
  ...
  const results = await Promise.allSettled([
    ...
    isRoutinesEnabled()
      ? runRoutineExecutorCycle({ baseUrl })
    ...
    // Operator read-model enrichment is durable, owner-scoped, and strictly
    // BYOK-only. Raw notifications remain the source-of-truth fallback while
    // this outbox sweep is delayed, disabled, or transiently unavailable.
    processOperatorProjectionBatch({
      workerId: `operator-projection:${crypto.randomUUID()}`,
      limit: 25,
      leaseSeconds: 120,
    }),
  ...
  for (const [i, result] of results.entries()) {
    if (result.status === "rejected") {
      ...
      const releaseResult =
        i === 1 && isCloudUsageHoldReleaseJobResult(result.value)
          ? result.value
          : null;
...
function isCloudUsageHoldReleaseJobResult(
  value: unknown,
): value is CloudUsageHoldReleaseJobResult {
...
async function runHourlyJobs(): Promise<void> {
  const results = await Promise.allSettled([
    ...
    (async () => {
      ...
      const backfillFlag = (getEnv("EXECUTED_BUNDLE_BACKFILL") || "")
        .toLowerCase();
  ...
  for (const [i, result] of results.entries()) {
    if (result.status === "rejected") {
      ...
      i === 0 && typeof result.value === "object" && result.value !== null &&
      "degraded" in result.value &&
      (result.value.degraded || result.value.errors.length > 0)
    ) {
