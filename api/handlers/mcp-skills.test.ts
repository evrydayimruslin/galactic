...
function installMcpSkillHarness(): {
  ...
  return {
    ...
    entitlementChecks: () => proEntitlementChecks,
  };
}

Deno.test("MCP tools/list no longer exposes skill SDK tools", async () => {
  const harness = installMcpSkillHarness();
  try {
    ...
    assertEquals(
      harness.entitlementChecks(),
      0,
      "tools/list must remain available before membership",
    );
  } finally {
    harness.cleanup();
  }
...
Deno.test("MCP serves full skills_md for free with no preview gating", async () => {
  ...
  try {
    ...
    assertEquals(
      harness.entitlementChecks(),
      0,
      "initialize and public resources must remain available before membership",
    );
  } finally {
    harness.cleanup();
  }
...
Deno.test("MCP tools/call rejects removed skill SDK tools", async () => {
  ...
  try {
    ...
    assertEquals(
      harness.entitlementChecks(),
      2,
      "every tools/call must enforce active membership",
    );
  } finally {
    harness.cleanup();
  }
