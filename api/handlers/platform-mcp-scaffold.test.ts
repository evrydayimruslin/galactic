...
Deno.test("platform scaffold: policy flag adds policy file and galactic.yaml hook", async () => {
  ...
  const galacticYaml = byPath.get("galactic.yaml")?.content;

  assertExists(index);
  assertExists(policy);
  assertExists(galacticYaml);
  ...
  const resolution = await compileGalacticAgentYaml(galacticYaml);
  const manifest = resolution.compiledManifest as {
    access_policy?: { mode?: string; module?: string; export?: string };
  };
  assertEquals("skills" in manifest, false);
