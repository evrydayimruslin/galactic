// WO-6.1: the CONCEPTS binding's wiring contract in the dynamic sandbox.
//
...
interface CapturedConceptsRuntime {
  setup: string;
  envKeys: string[];
...
class ConceptsTestSession {
  dup(): ConceptsTestSession {
    return this;
  }
...
function installHarness(): {
  captured: CapturedConceptsRuntime;
  restore(): void;
} {
  const captured: CapturedConceptsRuntime = {
    setup: "",
    envKeys: [],
  ...
  const testSessions = new Map<string, ConceptsTestSession>();
  const getTestSession = (name: string): ConceptsTestSession => {
    let session = testSessions.get(name);
    if (!session) {
      session = new ConceptsTestSession();
  ...
  Deno.env.set("AGENT_CALLER_SECRET", "concepts-runtime-test-secret");
  ...
  globalThis.__env = {
    ...
    AGENT_CALLER_SECRET: "concepts-runtime-test-secret",
    TRUST_SIGNING_SECRET: "concepts-runtime-trust-secret",
    // deno-lint-ignore no-explicit-any
  } as any;
  globalThis.__ctx = {
    exports: {
      ...
      ConceptsBinding: (input: any) => {
        ...
        return {
          ...
          describe: () =>
            Promise.resolve({ concept: { slug: "s", status: "active" } }),
        };
      },
      ...
      TestConceptsBinding: (input: any) => {
        ...
        if (
          ...
          throw new Error(
            "test concepts binding received an unknown session",
          );
        }
        ...
        return {
          ...
          describe: () =>
            Promise.resolve({
              concept: { slug: "ul-test-concept", status: "provisional" },
            }),
        };
      },
    },
...
function config(overrides: Partial<RuntimeConfig> = {}): RuntimeConfig {
  return {
    ...
    code: "",
    permissions: [],
    userApiKey: "provider-key-must-not-enter-concepts-binding",
    user: {
    ...
    authToken: "ul_human_bearer_must_not_enter_concepts_binding",
    workerSecret: "platform-worker-secret-must-not-enter-concepts-binding",
    d1DataService: null,
    memoryService: null,
    envVars: {},
    ...overrides,
...
Deno.test("dynamic concepts: production wiring freezes (appId, userId, userEmail) host-side", async () => {
  ...
  try {
    ...
    assert(harness.captured.envKeys.includes("CONCEPTS"));
    ...
    // The owner email rides the props for BYOK embedding-route resolution.
    assertEquals(
      harness.captured.productionProps?.userEmail,
      "owner@example.test",
    );
    // The generated SDK surface exists and speaks the authority language.
    assert(harness.captured.setup.includes("concepts:"));
    assert(harness.captured.setup.includes("CONCEPTS.about("));
    assert(harness.captured.setup.includes("CONCEPTS.suggest("));
    assert(
      harness.captured.setup.includes(
        "galactic.concepts.describe requires database.write authority",
      ),
    );
    assert(
      harness.captured.setup.includes(
        "galactic.concepts.about requires database.read authority",
      ),
    );
  } finally {
    harness.restore();
...
Deno.test("dynamic concepts: no declared database authority, no binding", async () => {
  const harness = installHarness();
  try {
...
Deno.test("dynamic concepts: gx.test mode uses only the deterministic stub", async () => {
  const harness = installHarness();
  try {
