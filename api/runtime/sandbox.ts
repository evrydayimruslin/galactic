...
export interface RuntimeConfig {
  ...
  /**
   * Exact immutable executable version selected by the lifecycle-gated app
   * row. Canonical member releases set this; legacy releases continue through
   * the compatibility `latest` pointer until they are republished.
   */
  immutableBundleVersion?: string;
  /**
   * Content-addressed executable identity for canonical releases. This is
   * stronger than the semantic-version key and takes precedence whenever set.
   */
  ...
  permissions: string[];
  /**
   * Exact per-function authority from galactic.yaml's compiled manifest.
   * null/undefined preserves legacy app-level behavior; [] denies every
   * effect-backed capability. Production runtimes treat this as a second,
   * narrower gate in addition to `permissions`.
   */
  ...
  // Exact, host-only canned HTTP responses for gx.test. These fixtures are
  // never exposed to tenant code and there is no live-network fallback.
  httpFixtures?: HttpTestFixtureConfig | null;
  // Credential declarations only (key -> destination), never credential
  // values. TestCredentialBinding uses this to preserve production's
  // destination binding while returning a canned response.
  testCredentialDestinations?: Record<string, string>;
  ...
  slotBindings?: Array<
    { slot: string; targetAppId: string; functions: string[] }
  >;
...
export interface ExecutionResult {
  ...
  /**
   * Host-authenticated marker that the requested exported function was found
   * and entered. Unlike dynamicWorkerInvoked, this stays false when loader or
   * dispatch fails before tenant code begins.
   */
  functionInvoked?: boolean;
  /**
   * Bounded, deduplicated basic-conformance effect IDs observed by gx.test.
   * Present for test-mode executions, including failed runs. Values describe
   * only platform operation classes and never include tenant-controlled data.
   */
  ...
  try {
    ...
    const sdk = {
      ...
      embed: async (request: { input?: unknown; model?: unknown }) => {
        ...
        if (!input) {
          throw new Error(
            "galactic.embed requires a non-empty string `input`.",
          );
        }
