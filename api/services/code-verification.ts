...
export async function readVersionSourceFiles(
  ...
  const hasAuthoredGalacticDocument = relativePaths.some((relativePath) => {
    const cleanPath = relativePath.startsWith("_source_")
      ? relativePath.replace(/^_source_/, "")
      : relativePath;
    return cleanPath === "galactic.yaml";
  });
  ...
  for (const relativePath of relativePaths) {
    ...
    // galactic.yaml is the authored source of truth. manifest.json beside it
    // is a server-compiled runtime artifact; returning both would make an exact
    // gx.download → gx.test round trip fail the ambiguity check and would
    // misrepresent generated bytes as developer-authored source.
    if (
      hasAuthoredGalacticDocument &&
      !isOriginal &&
...
export interface VerificationVerdict {
  ...
  integrity: {
    ...
    /**
     * Whether the running ESM bytes match the executable hash in signed
     * VersionTrust. Null only for legacy records that predate that binding.
     */
    executed_artifact_match: boolean | null;
...
export async function buildVerificationVerdict(
  ...
  // A valid platform HMAC is not enough on its own: the signed envelope must
  // describe this exact live subject. Otherwise a legitimate envelope copied
  // from another app, version, or runtime could make unrelated bytes appear
  // verified.
  const trustSubjectValid = Boolean(
    trust &&
      trust.app_id === app.id &&
      trust.version === version &&
      trust.runtime === (app.runtime || "deno"),
  );
  const signatureValid = trustSubjectValid && trust
    ? await verifyVersionTrustSignature(trust)
    : false;
  const verifiedTrust = signatureValid ? trust : null;
  const executedArtifactMatch = verifiedTrust?.executable_hash
    ? executedCode !== null &&
      await sha256Hex(executedCode) === verifiedTrust.executable_hash
    : null;
  ...
  if (openCode && verifiedTrust) {
    try {
      const matched = await openCodeFilesMatch(
        app.id,
        version,
        verifiedTrust,
      );
  ...
  const verified = signatureValid && executedOk &&
    // Historical trust records predate a signed executable digest. They remain
    // visible, but cannot prove which private runtime bytes actually execute
    // and therefore must never receive a green gx.verify verdict.
    executedArtifactMatch === true &&
    (openCode ? filesMatch === true : true);
  const integrity = {
    executed_bundle_status: executedStatus,
    executed_bundle_ok: executedOk,
    executed_artifact_match: executedArtifactMatch,
    published_signature_valid: signatureValid,
    manifest_hash: verifiedTrust?.manifest_hash ?? null,
    description_hash: verifiedTrust?.description_hash ?? null,
    artifact_hash: verifiedTrust?.artifact_hash ?? null,
    artifact_count: verifiedTrust
      ? Object.keys(verifiedTrust.artifact_hashes).length
      : 0,
    // Legacy signatures covered only the payload, not these header fields.
    // Present signer identity and time only from the protected V2 envelope.
    signer: verifiedTrust?.signature.envelope_version === 2
      ? verifiedTrust.signature.signer
      : null,
    signed_at: verifiedTrust?.signature.envelope_version === 2
      ? verifiedTrust.signature.signed_at
      : null,
  };
