...
function sanitizeRoutineUserMetadata(
  ...
  return Object.fromEntries(
    Object.entries(input).filter(([key]) => !isServerOwnedRoutineMetadataKey(key)),
  );
}
...
export function normalizeRoutineCreateInput(
  ...
  return {
    routine: {
      composer_app_id: optionalString(input.composer_app_id, 'composer_app_id', 80) ?? null,
      composer_app_slug: optionalString(input.composer_app_slug, 'composer_app_slug', 160) ??
        null,
      template_id: templateId,
      template_version: optionalString(input.template_version, 'template_version', 80) ?? null,
      name,
      ...
      created_by_trace_id: optionalString(input.created_by_trace_id, 'created_by_trace_id', 80) ??
        null,
...
export async function updateRoutine(
  ...
  if ('description' in input) {
    updates.description = optionalString(input.description, 'description', 500) ?? null;
  }
  if ('intent' in input) {
  ...
  const [routine] = await patchRows<RoutineSummary>(
    `user_routines?id=eq.${encodeURIComponent(routineId)}&user_id=eq.${userId}&deleted_at=is.null`,
    updates,
    ROUTINE_SELECT,
  );
  ...
  if (
    ...
    ).catch((err) => console.error('[CAPACITY] Failed to release activation slot:', err));
  }
  return updated;
}
...
export async function approveRoutineCapabilities(
  ...
  await patchRows<RoutineCapabilityRow>(
    `routine_capabilities?routine_id=eq.${encodeURIComponent(routineId)}&user_id=eq.${
...
export async function deleteRoutine(
  ...
  const res = await fetch(
    supabaseUrl(
      `user_routines?id=eq.${encodeURIComponent(routineId)}&user_id=eq.${userId}`,
    ),
    {
      method: 'PATCH',
...
export async function recordRoutineRunStep(params: {
  ...
  const [step] = await insertRows<RoutineRunStepRow>('routine_run_steps', {
    ...
    status,
    completed_at: status === 'succeeded' || status === 'failed' || status === 'skipped'
