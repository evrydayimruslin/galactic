...
Deno.test('createGrant: re-proposing an active grant never resets the spend window', async () => {
  ...
  const handler: Handler = (url, init) => {
    if (url.pathname.endsWith('/apps')) {
      return jsonResponse([{
        id: url.searchParams.get('id')?.includes('caller') ? 'app-caller' : 'app-target',
        owner_id: 'user-1',
        visibility: 'public',
      }]);
    }
    ...
    const handler: Handler = (url) => {
      if (url.pathname.endsWith('/apps')) {
        ...
        return jsonResponse([{
          ...
          owner_id: forbidden === 'cross-owner' && !isCaller ? 'user-2' : 'user-1',
          visibility: forbidden === 'public' && !isCaller ? 'public' : 'private',
          slug: null,
        }]);
      }
      return jsonResponse([]);
