...
export interface RuntimeModuleImportAnalysis {
  /** Literal module specifiers that remain in the prepared executable. */
  specifiers: string[];
  /**
   * True when import()/require() survives with a computed target. Its runtime
   * bytes cannot be included in the release identity and must fail closed.
   */
  hasComputedLoad: boolean;
}
...
/**
 * Inspect a prepared, bundled executable for module loads that will still be
 * resolved at runtime. Local authored imports should have disappeared into the
 * single-file bundle; any remaining non-platform specifier names code whose
 * bytes are outside the signed release.
 */
export async function analyzeRuntimeModuleImports(
  code: string,
  filename = "index.js",
): Promise<RuntimeModuleImportAnalysis> {
  ...
  let hasComputedLoad = false;
  const isRuntimeRequireName = (value: string): boolean =>
    value === "require" || value === "__require";

  const recordLiteral = (node: tsTypes.Expression | undefined): boolean => {
  ...
  const visit = (node: tsTypes.Node): void => {
    if (
      ...
      if (!recordLiteral(node.moduleReference.expression)) {
        hasComputedLoad = true;
      }
    } else if (ts.isCallExpression(node)) {
      if (node.expression.kind === ts.SyntaxKind.ImportKeyword) {
        if (!recordLiteral(node.arguments[0])) hasComputedLoad = true;
      } else if (
        ts.isIdentifier(node.expression) &&
        isRuntimeRequireName(node.expression.text)
      ) {
        if (!recordLiteral(node.arguments[0])) hasComputedLoad = true;
      }
    } else if (
      ts.isIdentifier(node) &&
      isRuntimeRequireName(node.text)
    ) {
      const parent = node.parent;
      // A direct require("literal") was handled above. Any other occurrence
      // can alias, reflect, or otherwise defer a module target beyond the
      // release-bound graph.
      if (
      ...
      isRuntimeRequireName(node.argumentExpression.text)
    ) {
      hasComputedLoad = true;
    }
...
export async function parseTypeScript(
  ...
  try {
    ...
    ts.forEachChild(sourceFile, (node: tsTypes.Node) => {
      if (
        ...
        for (const element of node.exportClause.elements) {
          ...
          if (
            originalDecl && !functions.some((f) => f.name === element.name.text)
          ) {
            ...
            node.moduleSpecifier &&
            !functions.some((f) => f.name === element.name.text)
          ) {
            // A named re-export is a real public entrypoint even though this
            // single-file parser cannot inspect its declaration. The strict
            // build resolves the module, and authoritative contracts supply
            // its schema; do not reject normal modular entry files.
            functions.push({
    ...
    try {
      ...
      parseWarnings.push(
        `Inference analysis failed: ${
          infErr instanceof Error ? infErr.message : String(infErr)
        }`,
      );
...
function isExported(node: tsTypes.Node): boolean {
  ...
  return modifiers?.some((m: tsTypes.ModifierLike) =>
...
function parseFunctionDeclaration(
  node: tsTypes.FunctionDeclaration,
  sourceFile: tsTypes.SourceFile,
): ParsedFunction | null {
  const ts = getLoadedTs();
  ...
  const isAsync =
    node.modifiers?.some((m: tsTypes.ModifierLike) =>
      m.kind === ts.SyntaxKind.AsyncKeyword
    ) ?? false;

  // Parse parameters
  const parameters = node.parameters.map((param) =>
    parseParameter(param, jsDoc, sourceFile)
  );

  // Parse return type
  const returnType = node.type
    ? node.type.getText(sourceFile)
    : (isAsync ? "Promise<void>" : "void");
...
function parseVariableFunction(
  ...
  const isAsync =
    fn.modifiers?.some((m: tsTypes.ModifierLike) =>
      m.kind === ts.SyntaxKind.AsyncKeyword
    ) ?? false;
...
function parseParameter(
  param: tsTypes.ParameterDeclaration,
...
function parseInterfaceDeclaration(
  node: tsTypes.InterfaceDeclaration,
  sourceFile: tsTypes.SourceFile,
): JsonSchema {
  const ts = getLoadedTs();
...
function parseTypeAlias(
  node: tsTypes.TypeAliasDeclaration,
  sourceFile: tsTypes.SourceFile,
): JsonSchema {
...
function typeNodeToJsonSchema(
  typeNode: tsTypes.TypeNode | undefined,
  sourceFile: tsTypes.SourceFile,
): JsonSchema {
  const ts = getLoadedTs();
  if (!typeNode) {
  ...
  if (ts.isTypeReferenceNode(typeNode)) {
    ...
    if (typeName === "Record" && typeNode.typeArguments?.length === 2) {
      return {
        type: "object",
        additionalProperties: typeNodeToJsonSchema(
  ...
  if (ts.isTupleTypeNode(typeNode)) {
    return {
      type: "array",
      items: typeNode.elements.map((el: tsTypes.TypeNode) =>
        typeNodeToJsonSchema(el, sourceFile)
      ),
    };
  }
  ...
  if (ts.isUnionTypeNode(typeNode)) {
    const types = typeNode.types.map((t) =>
      typeNodeToJsonSchema(t, sourceFile)
    );
...
/**
 * Preserve source positions and executable punctuation while blanking text
 * that cannot invoke a capability. Permission inference is a conservative
 * admission signal, but comments, docs, regexes, and string literals must not
 * silently widen an authored authority contract.
 */
function maskPermissionInertText(code: string): string {
  const ts = getLoadedTs();
  ...
  const inertTokens = new Set<tsTypes.SyntaxKind>([
  ...
  for (
    ...
    if (!inertTokens.has(token)) continue;
...
function inferPermissions(code: string): string[] {
  const executableCode = maskPermissionInertText(code);
  const permissions: string[] = [];

  // Storage operations
  if (new RegExp(`${SDK}\\.(store|batchStore)`).test(executableCode)) {
    permissions.push("storage:write");
  }
  if (new RegExp(`${SDK}\\.(load|list|query|batchLoad)`).test(executableCode)) {
    permissions.push("storage:read");
  }
  if (new RegExp(`${SDK}\\.(remove|batchRemove)`).test(executableCode)) {
    permissions.push("storage:delete");
  }

  // Memory operations
  if (new RegExp(`${SDK}\\.remember`).test(executableCode)) {
    permissions.push("memory:write");
  }
  if (new RegExp(`${SDK}\\.recall`).test(executableCode)) {
    permissions.push("memory:read");
  }

  // AI operations
  if (new RegExp(`${SDK}\\.ai`).test(executableCode)) {
    permissions.push("ai:call");
  }
  if (new RegExp(`${SDK}\\.embed`).test(executableCode)) {
  ...
  if (
    new RegExp(`${SDK}\\.compute\\b`).test(executableCode) ||
    new RegExp(
  ...
  // Network operations
  if (/\bfetch\s*\(/.test(executableCode)) {
...
export function fromSkillsParsed(skills: ParsedSkills): ParseResult {
  return {
    functions: skills.functions.map((fn) => ({
      name: fn.name,
      description: fn.description,
      parameters: Object.entries(fn.parameters as Record<string, JsonSchema>)
        .map(([name, schema]) => ({
          name,
