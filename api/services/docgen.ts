...
export function generateSkillsMd(
  ...
  options: { includeExamples?: boolean; includePermissions?: boolean; pricingConfig?: { default_price_light?: number; functions?: Record<string, number>; products?: Array<{ id: string; name: string; price_light: number; description?: string }> } } = {}
): string {
  ...
  if (pc && (pc.default_price_light || pc.functions || (pc.products && pc.products.length > 0))) {
    ...
    if (pc.default_price_light) {
      lines.push(`Default: **${formatLight(pc.default_price_light)}** per call`);
    } else {
    ...
    if (pc.products && pc.products.length > 0) {
      ...
      for (const p of pc.products) {
        lines.push(`| \`${p.id}\` — ${p.name} | ${formatLight(p.price_light)} | ${p.description || ''} |`);
      }
    }
    lines.push('');
