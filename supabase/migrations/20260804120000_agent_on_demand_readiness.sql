-- Treat schedule-free Agents as healthy, callable fleet members.
--
-- Normal deployment activation, required developer settings, and declared
-- inference configuration still gate readiness. Galactic inbox reporting and
-- routine capability/grant checks apply only when a managed routine exists.
--
-- Existing missing-primary-routine operator items are retained as recovered
-- audit history, but disappear from every active Attention projection.

UPDATE public.operator_items
SET lifecycle_state = 'recovered',
    recovered_at = now(),
    recovery_reason = 'Primary routines are optional.',
    updated_at = now()
WHERE lifecycle_state = 'active'
  AND diagnosis->>'code' = 'AGENT_PRIMARY_ROUTINE_MISSING';
            ...
            WHEN coalesce(
              ...
              THEN EXISTS (
                ...
                WHERE secrets.user_id = p_user_id
                  ...
                  AND secrets.value_encrypted <> ''
              )
            ELSE
              jsonb_typeof(agents.env_vars) = 'object'
              AND agents.env_vars ? settings.key
              AND jsonb_typeof(agents.env_vars->settings.key) = 'string'
              AND agents.env_vars->>settings.key <> ''
          END
        )
...
readiness AS (
  SELECT
    inputs.*,
    CASE
      WHEN inputs.deployment_state = 'disabled' THEN 'disabled'
      ...
      WHEN inputs.managed_count > 0
           AND NOT inputs.reporting_configured THEN 'setup_required'
      ...
      WHEN inputs.managed_count > 0
           AND inputs.executable_count = 0
           AND inputs.paused_count > 0 THEN 'paused'
      WHEN inputs.managed_count > 0
           AND inputs.executable_count = 0 THEN 'disabled'
      ELSE NULL
  ...
  SELECT
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'label', CASE
        ...
        WHEN positioned.exclusion_reason = 'setup_required'
          THEN 'Setup required'
        ...
        WHEN positioned.managed_count = 0 THEN 'Available on demand'
      ...
      'basis', CASE
        ...
        WHEN positioned.managed_count = 0 THEN 'readiness'
...
SELECT
  ...
  CASE
    WHEN projected.managed_count = 0
         AND projected.exclusion_reason IS NULL THEN 'active'
    ELSE projected.state
  END,
  CASE
    WHEN projected.managed_count = 0
         AND projected.exclusion_reason IS NULL THEN 'healthy'
    ELSE projected.health
  END,
